...
export default function ValidatePassword({password}) {
    ...
    useEffect(() => {
        ...
        } else if (password.length > 0 && !/\d/.test(password)) { // contains at least one digit
            setPasswordError({ message: "Password must contain at least 1 digit!", status: "error"})
        } else if (password.length > 0 && !/[A-Z]/.test(password)) {// contains at least one uppercase letter
            setPasswordError({ message: "Password must contain at least 1 uppercase letter!", status: "error"})
        }
        else if (password.length > 0 && !/[a-z]/.test(password)) {// contains at least one lowercase letter
            setPasswordError({ message: "Password must contain at least 1 lowercase letter!", status: "error"})
        }
        else if (password.length > 0 && !/[^A-Za-z0-9]/.test(password)) { // contains at least one special
